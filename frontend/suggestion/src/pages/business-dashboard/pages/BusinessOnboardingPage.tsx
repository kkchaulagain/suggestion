...
export default function BusinessOnboardingPage() {
  ...
  const pagesPreview = bundle.pages as OnboardingPagePayload[]
  ...
  const runSetup = useCallback(async () => {
    if (!siteType) return
    const { forms, pages } = getStarterBundle(siteType)
    setRunning(true)
    setError(null)
    try {
      await axios.post(
  ...
  }, [siteType, authHeaders.Authorization, refetchBusiness])
    ...
    <div className="space-y-6 max-w-2xl mx-auto p-4">
      ...
      {step === 'preview' && siteType && (
        <Card padding="lg">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm mb-4">
            Based on your selection, we’ll create the following. You can edit or add more after setup.
          </p>
          ...
          {pagesPreview.length > 0 && (
            <div className="mb-4">
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Pages</p>
              <ul className="list-disc list-inside text-sm text-slate-600 dark:text-slate-400 space-y-1">
